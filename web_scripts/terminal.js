...
var Terminal=(function(){
  ...
  function CommandHist(){
    ...
    this.prev=function(current){
      ...
      if(this.pos==this.hist.length){return(this.current);}
      else{return(this.hist[this.pos]);}
    };
    ...
    this.next=function(current){
      ...
      if(this.pos<this.hist.length){this.pos++;}
      if(this.pos==this.hist.length){return(this.current);}
      else{return(this.hist[this.pos]);}
    };
    ...
    this.push=function(current){
      if(current&&current.length>0){
        if(this.hist.push){this.hist.push(current);}
        else{this.hist[this.hist.length]=current;}
      }
      this.current="";
  ...
  function Stdout(target,before){
    ...
    this.scrollBottom=function(){
      ...
      //this.target.scrollTo(0,term.scrollHeight);
    ...
    this.flushTEXT=function(){
      ...
      var word="",w,innerTag=sp;

      for(var i=0;i<this.buf.length;i++){
	w=this.buf.substr(i,1);
	if(w=='\n'){
	  if(word.length>0){
	    innerTag.appendChild(document.createTextNode(word));
	    word="";
	  }
	  ...
	  innerTag.appendChild(document.createElement("wbr"));
	} else if (w=='\b') {
          if (innerTag != sp) {
            if(word.length>0){
              innerTag.appendChild(document.createTextNode(word));
            ...
            sp.appendChild(innerTag)
            innerTag=sp;
          } else {
            innerTag = document.createElement("em");
          }
	} else{
	  word+=w;
	}
	...
	sp.appendChild(document.createTextNode(word));
      ...
      sp.className="stdout";
      this.target.insertBefore(sp,this.before);
      //document.body.insertBefore(sp,this.target);
      this.buf="";
    };
    ...
    this.flushHTML=function(){
      ...
      di.className="stdout";
      this.target.insertBefore(di,this.before);
      //document.body.insertBefore(di,this.target);
      this.buf="";
    };
  ...
  function Stdin(element){
    ...
    this.passwordMode=function(){
      this.saveHist=false
      this.element.type="password"
      this.focus()
    }

    this.regularMode=function(){
      this.saveHist=true
  ...
  var term={
    ...
    gid:function(element,id){
      ...
      else if(element.layers){return(element.layers[id]);}
    },
    keyHandlerList:[
    ...
    stdinfocus:function(){term.stdin.focus();},
    initHook:function(){},
    init:function(){
	...
	document.body.onclick=term.stdinfocus;
      ...
      term.stdin.focus();
      term.stdout=new Stdout(term.gid(document,"term"),term.stdin.element);
      if(term.header.length>0){term.stdout.puts(term.header);}
    ...
    history:function(){return(term.stdin.hist.hist);},
    commandCallBack:function(com){},
    showPrompt:function(){
      if (term.prompt != term.promptI && term.prompt != this.promptC) {
